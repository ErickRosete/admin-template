    ...
    return (
      <Layout title="Registro de usuario">
        <div>
          ...
          {this.state.clicked && 
            <Query  query={SEARCH_USER } variables= {{ email:this.state.email}} onCompleted={this.onQueryCompleted}>
              {({  loading, error, data }) => {
                  if (loading){
                    return(
                      <p>loading</p>
          ...
          <form className="product-form" onSubmit={this.onSubmitHandler}>
            <Grid container spacing={24} justify="center">
              ...
              <Grid item xs={12} md={6}>
                <Button type="submit" variant="contained" color="primary" autoFocus size="large">
                    Guardar
                </Button>
              </Grid>
            </Grid>
